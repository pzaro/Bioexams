...
st.markdown("""
    ...
    /* ΕΠΙΒΟΛΗ ΚΕΝΤΡΑΡΙΣΜΑΤΟΣ ΣΕ ΟΛΑ ΤΑ ΚΕΛΙΑ */
...
def find_first_number(s):
    # Καθαρίζουμε την είσοδο πριν ψάξουμε
    ...
    for metric_name, keywords in selected_metrics.items():
        for i, line in enumerate(lines):
            if any(key.upper() in line.upper() for key in keywords):
                ...
                # Ψάχνουμε στην ίδια γραμμή
                val = find_first_number(line)
                
                # Αν δεν βρούμε, ψάχνουμε μέχρι 5 γραμμές κάτω (Deep Scan)
                if val is None:
                    for offset in range(1, 6): # Κοιτάει i+1 έως i+5
...
def to_excel_with_chart(df, chart_fig):
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        worksheet.set_column('A:Z', 20, center_fmt)
    ...
    with c1:
        if fig:
            try:
                ...
                st.warning("Για γράφημα στο Excel χρειάζεται το 'kaleido'.")
